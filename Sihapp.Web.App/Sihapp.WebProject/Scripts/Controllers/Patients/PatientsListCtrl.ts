...
interface PatientsListScope extends ng.IScope {
    message: string;
    showAddPatientModal();
    customFieldsTypes: Sihapp.WebProject.Models.Common.TextValueVM[];
    patientItems: Sihapp.WebProject.Models.Patients.PatientItemVM[];
    pagination: Sihapp.WebProject.Models.Common.PaginationVM;
    openDeletePopUp(patient: Sihapp.WebProject.Models.Patients.PatientItemVM);
    ...
    searchText: string;
    processing: boolean;
    validateAddPatient(): string;

    selectedPatient: Sihapp.WebProject.Models.Patients.PatientItemVM;
    searchPanel();
    delete();
    goToExpedient(id: System.Guid);
    reset: boolean;
    PatientsQty: number;

    addCustomFieldToView();
    customFieldsValuesToRequest: Sihapp.WebProject.Models.CustomFieldsVM[];
    presetFields: Sihapp.WebProject.Models.CustomFields.ExtraFieldsVM[];
    getPresetFields();

}

angularApp.controller('patientsListCtrl', function ($scope: PatientsListScope, $http: ng.IHttpService) {
    ...
    $scope.search = function () {
        ...
        $http({
            ...
            success(function (data: Sihapp.WebProject.Models.Patients.PageablePatientsVM, status, headers, config) {
    ...
    $scope.getPresetFields = function () {
        ...
        $http({
            ...
            success(function (data: Sihapp.WebProject.Models.CustomFields.ExtraFieldsVM[], status, headers, config) {
    ...
    $scope.openDeletePopUp = function (patient: Sihapp.WebProject.Models.Patients.PatientItemVM) {
        $scope.selectedPatient = patient;
        $('#deletePatientModal').modal('show');
    }

    $scope.goToExpedient = function (id: System.Guid) {
    ...
    $scope.addCustomFieldToView = function () {
        var newCustomField: Sihapp.WebProject.Models.CustomFieldsVM;
        newCustomField = <Sihapp.WebProject.Models.CustomFieldsVM>{};
        newCustomField.FieldType = $scope.customFieldsTypes[0].Value;
        newCustomField.FieldText = "";
        newCustomField.AllowAllEntities = true;
        $scope.customFieldsValuesToRequest.push(<Sihapp.WebProject.Models.CustomFieldsVM>newCustomField);
    }
